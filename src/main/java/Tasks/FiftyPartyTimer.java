...
public class FiftyPartyTimer extends BukkitRunnable {
    private final ArmorStand armorStand;
    private int seconds;

    public FiftyPartyTimer(ArmorStand armorStand, int seconds) {
    ...
    public void run() {
        if (seconds == 0) {
            armorStand.remove();
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "mm mobs spawn party50 1 newlobby,-173.500,57,232.500");
            cancel();
            return;
        }
